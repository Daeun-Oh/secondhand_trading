...
import org.koreait.annotations.MockMember;
...
//                .andDo(print());
     ...
     * - @WithMockUser
     *
     * - @MockSecurityContextFactory 가짜 데이터 (
     */


    @Test
    @WithMockUser(authorities = "MEMBER")  // 로그인 상태로 테스트 (인가 확인)
    ...
    @Test
    @WithMockUser(authorities = "ADMIN")  // 로그인 상태로 테스트 (인가 확인)
    ...
    @Test
    @WithUserDetails(value = "user01@test.org")  // 해당 멤버를 조회해서 사용 -> 문제점: DB에 없는 회원이면 테스트를 못함 (무조건 회원가입을 해야 됨)
